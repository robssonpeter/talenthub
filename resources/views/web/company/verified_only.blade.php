    ...
    <section class="pb80 bg-gray" id="company-profile">
        <div class="container">
            <div class="company-profile text-center">
                <img src="{{asset('assets/images/no-touch.svg')}}" height="200" alt="">
                <p class="mt-2">This section is only accessible by verified employers</p>
                <div>
                    @isset($company)
                        @if($company->has('verification_attempt'))
                            <a href="{{ route('company.verify') }}" class="btn btn-success">Check Verification Status</a>
                        @else
                            <a href="{{ route('company.verify') }}" class="btn btn-success">Verify Account</a>
                        @endif
                    @endisset
                </div>
