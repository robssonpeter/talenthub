...
$(document).ready(function () {
  ...
  $('#editDetails,#editValueProposition').summernote({
    minHeight: 200,
    toolbar: [['style', ['bold', 'italic', 'underline', 'clear']], ['font', ['strikethrough']], ['para', ['li','ol', 'ul',,'paragraph']]]
  });
  ...
  $(document).on('submit', '#editCompanyForm', function (e) {
    $('#btnSave').prop('disabled', true);
    processingBtn('#editCompanyForm', '#submit-form', 'loading');
    if ($('#editDetails').summernote('isEmpty')) {
      displayErrorMessage('Employer Details field is required.');
      e.preventDefault();
      $('#btnSave').attr('disabled', false);
      processingBtn('#editCompanyForm', '#submit-form');
      return false;
    }
  });
