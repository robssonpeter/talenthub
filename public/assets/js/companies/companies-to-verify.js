...
$(tableName).DataTable({
  ...
  columns: [{
    ...
    data: function data(row) {
      ...
      let link = '<a class="btn btn-primary action-btn w-100 dropdown-toggle text-white" type="button" data-toggle="dropdown"  aria-haspopup="true" aria-expanded="false">'+attachment+'s</a>\n' +
    ...
    data: function data(row) {
        let action = '<a class="btn btn-primary action-btn w-100 dropdown-toggle text-white" type="button" data-toggle="dropdown"  aria-haspopup="true" aria-expanded="false">'+actions+'</a>\n' +
            '    <div class="dropdown-menu w-100px">\n' +
            '        <a class="dropdown-item text-success verify-btn" id="'+row.id+'" href="#">'+markVerified+'</a>\n' +
            '        <a class="dropdown-item text-warning reject-btn" data-id="'+row.id+'" href="#">'+rejectAct+'</a>\n' +
...
$(document).on('click', '.reject-btn', function (event) {
    ...
    if (reason.length) {
        $.ajax({
            url: rejectionUrl.replace('***', companyId) ,
            data: { reason: reason},
            method: 'post',
            cache: false,
            success: function success(result) {
                displaySuccessMessage(rejectSuccess);
            },
            error: function error(result) {
                displayErrorMessage(result.responseJSON.message);
            }
        ...
        swal('Error', 'You must specify the reason for rejection', 'error')
    }
});
