...
$(tableName).DataTable({
  ...
  columns: [{
    ...
    data: function data(row) {
      return '<a href="' + companyUrl + '/' + row.company.unique_id + '">' + row.company.user.first_name.replace('&amp;', '&') + '</a>';
    },
    name: 'company_id'
  }, {
    data: function data(row) {
    ...
    data: function data(row) {
      ...
      let appLink = applicationUrl.replace('**job_id**', row.id);
      return '<a title="" class="btn btn-dark action-btn mb-1 mt-1" href="'+appLink+'" data-toggle="tooltip" data-placement="bottom" data-original-title="Job Applications ('+row.applications_count+') ">\n' +
          '            <i class="fa fa-users"></i>\n' +
          '   </a>'+prepareTemplateRender('#jobActionTemplate', data);
    },
    name: 'id'
  }],
