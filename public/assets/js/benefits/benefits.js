...
$(tableName).DataTable({
  ...
  columns: [{
    ...
    data: function data(row) {
      ...
      return prepareTemplateRender('#benefitActionTemplate', data);
    },
    name: 'id'
  }]
});
...
$(document).on('click', '.edit-btn', function (event) {
  var benefitId = $(event.currentTarget).data('id');
  renderData(benefitId);
});

window.renderData = function (id) {
    /*alert(benefitUrl + id + '/edit');
    return;*/
  $.ajax({
    ...
    success: function success(result) {
      if (result.success) {
        $('#benefitId').val(result.data.id);
        $('#editBenefit').val(result.data.name);
